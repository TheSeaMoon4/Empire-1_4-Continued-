...
namespace FactionColonies
{
    public class DesignUnitsWindow : MilitaryWindow
    {
        ...
        public override void DrawTab(Rect rect)
        {
            ...
            Rect ChangeXeno = new Rect(425, ResetButton.y, SavePawn.width, SavePawn.height);
            ...
            if (Widgets.ButtonText(ChangeXeno, "changeUnitXenoButton".Translate()))
            {
                ...
                foreach (PawnKindDef def in DefDatabase<PawnKindDef>.AllDefsListForReading.Where(def => def.race.HasModExtension<XenotypeProperties>() && !xenos.Contains(def.race.label) && faction.raceFilter.Allows(def.race)))
                {
                    if (def.race == ThingDefOf.Human && def.LabelCap != "Colonist") continue;
                    xenos.Add(def.race.label);

                    XenotypeProperties xenoProps = def.race.GetModExtension<XenotypeProperties>();
                    string optionStr = xenoProps.xenoLabel.CapitalizeFirst() + " - Cost: " + Math.Floor(def.race.BaseMarketValue * FactionColonies.militaryXenoCostMultiplier);
                    options.Add(new FloatMenuOption(optionStr, delegate
                    {
                        selectedUnit.pawnKind = def;
                        selectedUnit.generateDefaultPawn();
